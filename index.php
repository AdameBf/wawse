...
if (!isset($_SESSION['id']))
{
	if (isset($_COOKIE['wa_sch_edit_lang']) OR isset($_POST['wa_sch_edit_lang']))
	{
		if(isset($COOKIE['wa_sch_edit_lang']))
		{
			...
			{
			// The visitor is a nasty guy that edited the cookie. We'll right load the English string file; but since the cookie was edited, let's change it to 'en'.
			include('includes/strings/en.php');
		...
		else if (isset($_POST['wa_sch_edit_lang']))
		{
			...
			{
			// The visitor is a nasty guy that edited the form. We'll just load the English string file.
			include('includes/strings/en.php');
			setcookie('wa_sch_edit_lang', 'en', time() + 365*24*3600, null, null, false, true);
			}
		}
...
<ul>
	...
	?>
<ul>
	<li><a href="version-history.php"><?php echo $str['sch_editor_changelog']; ?></a></li>
</ul>
<?php
